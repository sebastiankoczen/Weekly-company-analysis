...
# Your Perplexity API key (will be set via GitHub Secrets for security)
...
def parse_perplexity_response(response_text):
    """
    Parse the structured response from Perplexity into CompanyAnalysis objects
    """
    companies = []
    ...
    for block in company_blocks[1:]:  # Skip first empty split
        ...
        for sit_num in range(1, 5):
            ...
            if match:
                ...
                company.situations[sit_num]["points"] = points[:3]  # Ensure max 3
                company.situations[sit_num]["sources"] = sources
...
def create_excel_report(companies, week_num, output_path):
    """
    Create an Excel file with formatted analysis and clickable links
    """
    try:
        import pandas as pd
        ...
        for company in companies:
            for sit_num in range(1, 5):
                ...
                row = {
                    ...
                    'Sources': ' | '.join(situation['sources'])
                }
                data.append(row)
        ...
        score_fill_high = PatternFill(start_color="C6EFCE", end_color="C6EFCE", fill_type="solid")  # Light green
        score_fill_med = PatternFill(start_color="FFEB9C", end_color="FFEB9C", fill_type="solid")   # Light yellow
        score_fill_low = PatternFill(start_color="FFC7CE", end_color="FFC7CE", fill_type="solid")    # Light red
        ...
        ws.column_dimensions['B'].width = 25  # Situation
        ws.column_dimensions['C'].width = 8   # Score
        ws.column_dimensions['D'].width = 40  # Key Point 1
        ws.column_dimensions['E'].width = 40  # Key Point 2
        ws.column_dimensions['F'].width = 40  # Key Point 3
        ws.column_dimensions['G'].width = 50  # Sources
...
def generate_html_email(companies, week_num):
    """
    Generate a beautiful HTML email with the analysis table
    """
...
def get_perplexity_response(prompt_text):
    """Send a prompt to Perplexity API and get the response"""
    ...
    body = {
        ...
        "max_tokens": 8000,  # Increased for 10 companies x 4 situations
        "stream": False,
        "temperature": 0.2,
        "top_p": 0.9
    }
...
def send_html_email(subject, html_content, excel_path=None):
    """Send the report via email with HTML formatting and Excel attachment"""
    if not SEND_EMAIL:
...
def save_to_file(content, week_num):
    """Save the raw report to a text file"""
...
def main():
    ...
    print("=" * 70)
    print("🤖 WEEKLY AI COMPANY ANALYSIS REPORT")
    print("=" * 70)
    ...
    try:
        ...
        if week_num == 0:
            ...
            print(f"📅 Auto-calculated week number: {week_num}")
        else:
            print(f"📅 Using manually set week number: {week_num}")
        ...
        print("🔍 Requesting analysis from Perplexity AI (this may take 2-3 minutes)...")
