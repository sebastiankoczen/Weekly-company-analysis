"""
Weekly AI Company Analysis Report - Powered by Gemini + Google Search
Processes companies one-by-one, produces 1 consolidated email + Excel

WHY GEMINI:
- Google Search Grounding = real-time web search built into the API
- Same deep research capability as Perplexity Pro web interface
- Reliable, documented API feature (not broken like Perplexity API)
- Cost-effective: ~$0.35/month for 10 companies/week
"""
...
# Gemini model - both support Google Search grounding
# gemini-1.5-pro  = best quality, slightly slower
# gemini-2.0-flash = faster, cheaper, still excellent
GEMINI_MODEL = "gemini-1.5-pro"
COMPANIES_PER_WEEK = 2   # Keep at 2 for testing; increase to 10 when satisfied
    ...
    for idx, block in enumerate(company_blocks[1:], 1):
        ...
        for sit_num in range(1, 5):
            ...
            if match:
                ...
                company.situations[sit_num]["sources"] = sources[:3]  # 3 sources
    ...
    for company in companies:
        for sit_num in range(1, 5):
            ...
            if situation['sources']:
                sources_links = []
                for idx, source in enumerate(situation['sources'][:3], 1):
...
def get_gemini_response(prompt_text):
    """
    Send prompt to Gemini API with Google Search Grounding enabled.
    Google Search Grounding = Gemini performs real-time Google searches
    and cites actual sources — equivalent to Perplexity Pro web interface.
    """
    ...
    # Google Search Grounding tool — the key feature that makes this work
    # This tells Gemini to search Google in real-time before answering
    google_search_tool = genai.protos.Tool(
        google_search_retrieval=genai.protos.GoogleSearchRetrieval(
            dynamic_retrieval_config=genai.protos.DynamicRetrievalConfig(
                mode=genai.protos.DynamicRetrievalConfig.Mode.MODE_DYNAMIC,
                dynamic_threshold=0.3  # Low threshold = searches more aggressively
            )
        )
    )
    ...
    try:
        print(f"🔍 Calling Gemini API with Google Search Grounding...")
        response = model.generate_content(prompt_text)

        content = response.text
        print(f"   ✅ Received {len(content)} characters")

        # Log grounding metadata if available (shows which sources were searched)
        try:
            if response.candidates[0].grounding_metadata:
                meta = response.candidates[0].grounding_metadata
                if hasattr(meta, 'grounding_chunks') and meta.grounding_chunks:
                    print(f"   🔗 Gemini searched {len(meta.grounding_chunks)} sources")
        except Exception:
...
def main():
    """Main function - processes companies one-by-one, sends 1 consolidated email"""
    print("=" * 70)
    print("🤖 WEEKLY COMPANY ANALYSIS - POWERED BY GEMINI + GOOGLE SEARCH")
    print("=" * 70)
    ...
    try:
        ...
        for i, company_name in enumerate(companies_this_week, 1):
            ...
            try:
                ...
                company_response = get_gemini_response(single_prompt)
        ...
        if not companies_data:
            ...
            print("   Check the raw .txt file for Gemini's response format")
            print("   Gemini may have used slightly different delimiters")
        ...
        # Cost estimate (Gemini 1.5 Pro pricing)
        # Input: $3.50/1M tokens, Output: $10.50/1M tokens (for >128K context)
        # Approximate: ~$0.007 per company for typical analysis
        estimated_cost = successful_count * 0.007
        print(f"💰 Estimated cost this run: ~${estimated_cost:.3f}")
        print(f"   ({successful_count} companies × ~$0.007 per company)")
        print(f"   Monthly estimate (10 companies/week): ~${0.007 * 10 * 4:.2f}")
        print()
